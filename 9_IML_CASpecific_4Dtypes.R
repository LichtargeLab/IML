...
drugs_name <- names(sort(avg_dr))[1:25]
...
foreach(dr=1:length(drugs_name), .errorhandling = "pass") %dopar% {
  ...
  if (dim(pheno_data_ca_dr)[1]>=30){ 
    for (m in 1:ncol(dt_comb)){
      ...
      common_id <- intersect(intersect(intersect(intersect(pheno_data_ca_dr$SANGER_MODEL_ID, row.names(dt1)), row.names(dt2)), row.names(dt3)), row.names(dt4))
      ...
      fold_data3 <- data.frame(dt3_fltr, label=as.numeric(pheno_data_ca_dr_fltr$LN_IC50))
      outer_folds3 <- cvTools::cvFolds(nrow(fold_data3), K=nfolds[1])
      
      fold_data4 <- data.frame(dt4_fltr, label=as.numeric(pheno_data_ca_dr_fltr$LN_IC50))
      ...
      # Save results
      # setwd("...")
      save(icncv_res, file = paste0(ca, "_", drugs_name[dr], "_all_ffs_res.RData"))
    }
  }
}
