...
foreach(drug=1:length(drugs_names), .errorhandling = "pass") %dopar% {
  ...
  for (m in 1:ncol(dt_comb)){
    ...
    common_id <- intersect(intersect(intersect(pheno_drug$SANGER_MODEL_ID, row.names(dt1)), row.names(dt2)), row.names(dt3))
    ...
    for (i in 1:nfolds[1]){
      ...
      integrated_atts <- c(nCV_atts1, nCV_atts2, nCV_atts3)
    ...
    # Save results
    # setwd("...")
    save(icncv_res, file = paste0(drugs_names[drug], "_", dt[1], "_", dt[2], "_", dt[3], "_ffs_res.RData"))
  }
}
