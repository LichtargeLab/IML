...
foreach(dr=1:length(drugs_name), .errorhandling = "pass") %dopar% {
  ...
  if (dim(pheno_data_ca_dr)[1]>=30){ 
    for (m in 1:ncol(dt_comb)){
      ...
      common_id <- intersect(intersect(intersect(pheno_data_ca_dr$SANGER_MODEL_ID, row.names(dt1)), row.names(dt2)), row.names(dt3))
      ...
      # Save results
      # setwd("...")
      save(icncv_res, file = paste0(ca, "_", drugs_name[dr], "_", dt[1], "_", dt[2], "_", dt[3], "_ffs_res.RData"))
    }
  }
}
