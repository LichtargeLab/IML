...
foreach(drug=1:length(drugs_names), .errorhandling = "pass") %dopar% {
  pheno_drug <- pheno_data[which(pheno_data$DRUG_NAME==drugs_names[drug]), ]
  for (m in 1:ncol(dt_comb)){
    # setwd("...")
    dt <- dt_comb[, m]
    dt1 <- get(load(paste0(dt[1],"_data.RData")))
    dt2 <- get(load(paste0(dt[2],"_data.RData")))
    dt3 <- get(load(paste0(dt[3],"_data.RData")))
    dt4 <- get(load(paste0(dt[4],"_data.RData")))
    
    common_id <- intersect(intersect(intersect(intersect(pheno_drug$SANGER_MODEL_ID, row.names(dt1)), row.names(dt2)), row.names(dt3)), row.names(dt4))
    dt1_fltr <- dt1[rownames(dt1)%in%common_id, ]
    dt2_fltr <- dt2[rownames(dt2)%in%common_id, ]
    dt3_fltr <- dt3[rownames(dt3)%in%common_id, ]
    dt4_fltr <- dt4[rownames(dt4)%in%common_id, ]
    
    pheno_drug_fltr <- pheno_drug[pheno_drug$SANGER_MODEL_ID%in%common_id, ]
    pheno_drug_fltr <- pheno_drug_fltr[match(rownames(dt1_fltr),  pheno_drug_fltr$SANGER_MODEL_ID),]
    fold_data1 <- data.frame(dt1_fltr, label=as.numeric(pheno_drug_fltr$LN_IC50))
    outer_folds1 <- cvTools::cvFolds(nrow(fold_data1), K=nfolds[1])
    
    fold_data2 <- data.frame(dt2_fltr, label=as.numeric(pheno_drug_fltr$LN_IC50))
    outer_folds2 <- cvTools::cvFolds(nrow(fold_data2), K=nfolds[1])
    
    fold_data3 <- data.frame(dt3_fltr, label=as.numeric(pheno_drug_fltr$LN_IC50))
    outer_folds3 <- cvTools::cvFolds(nrow(fold_data3), K=nfolds[1])
    
    fold_data4 <- data.frame(dt4_fltr, label=as.numeric(pheno_drug_fltr$LN_IC50))
    outer_folds4 <- cvTools::cvFolds(nrow(fold_data4), K=nfolds[1])
    ...
    for (i in 1:nfolds[1]){
      ...
      outer_train3 <- fold_data3[which(outer_folds3[[5]]!=i), ]
      outer_test3 <- fold_data3[which(outer_folds3[[5]]==i), ]
      
      outer_train4 <- fold_data4[which(outer_folds4[[5]]!=i), ]
      outer_test4 <- fold_data4[which(outer_folds4[[5]]==i), ]
      
      inner_folds1 <- cvTools::cvFolds(nrow(outer_train1), K=nfolds[2])
      inner_folds2 <- cvTools::cvFolds(nrow(outer_train2), K=nfolds[2])
      inner_folds3 <- cvTools::cvFolds(nrow(outer_train3), K=nfolds[2])
      inner_folds4 <- cvTools::cvFolds(nrow(outer_train4), K=nfolds[2])
      ...
      for (j in 1:nfolds[1]){
        for (t in 1:length(dt)){
          ...
          if (t == 1) {
            ...
            atts3[[j]] <-  genelist_sorted$gene[genelist_sorted$pvalue<0.05]
            atts_sig <-  rbind(atts_sig, genelist_sorted[genelist_sorted$pvalue<0.05, ])
          } else {
            atts4[[j]] <-  genelist_sorted$gene[genelist_sorted$pvalue<0.05]
            atts_sig <-  rbind(atts_sig, genelist_sorted[genelist_sorted$pvalue<0.05, ])
          }
        }
      ...
      nCV_atts3 <- Reduce(intersect, atts3) 
      nCV_atts4 <- Reduce(intersect, atts4)      
      ...
      outer_train_fltr <- cbind(outer_train1[, nCV_atts1, drop=FALSE], 
                                outer_train2[, nCV_atts2, drop=FALSE],
                                outer_train3[, nCV_atts3, drop=FALSE],
                                outer_train4[, nCV_atts4, drop=FALSE])
      outer_train_fltr$label <- outer_train1[, 'label']
      outer_test_fltr <- cbind(outer_test1[, nCV_atts1, drop=FALSE], 
                               outer_test2[, nCV_atts2, drop=FALSE], 
                               outer_test3[, nCV_atts3, drop=FALSE], 
                               outer_test4[, nCV_atts4, drop=FALSE])
      ...
      integrated_atts <- c(nCV_atts1, nCV_atts2, nCV_atts3, nCV_atts4)
      integrated_atts_sig <- atts_sig[which(atts_sig$gene%in%integrated_atts), ]
      integrated_atts_sig_sorted <- unique(integrated_atts_sig[order(integrated_atts_sig$pvalue), 'gene'])
      prev_r2 = 0; best_r2 = 0; best_features = integrated_atts_sig_sorted[1]
      if (length(integrated_atts_sig_sorted)>=2){
        for (f in integrated_atts_sig_sorted[2:length(integrated_atts_sig_sorted)]){
          best_features <- c(best_features, f)
          # Random forest
          rf_model <- randomForest::randomForest(outer_train_fltr[, best_features, drop=FALSE], outer_train1[, "label"])
          rf_pred <- stats::predict(rf_model, outer_test_fltr[, best_features, drop=FALSE])
          rf_r2_cur <- stats::cor(rf_pred, outer_test1[, "label"])^2
          if (rf_r2_cur > prev_r2){
            prev_r2 <- rf_r2_cur
          } else {
    ...
    # Save results
    # setwd("...")
    save(icncv_res, file = paste0(drugs_names[drug], "_all_ffs_res.RData"))
  }
}
